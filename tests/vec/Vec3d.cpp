...
int main(int argc, char *argv[])
{
	...
	{ // array mode referencing
		const Vec3d vec(1.4,2.5,5.6);
		Assertv(vec.equals(1.4,2.5,5.6), verbose);
	...
	{ // Nan
		{
			const Vec3d vec(Double::Nan, Double::Nan, Double::Nan);
			Assertv(vec.isNan(), verbose);
			Assertv(vec.hasNan(), verbose);
			Assertv(!vec.hasNoNan(), verbose);
		}
		{
			const Vec3d vec(0.6, Double::Nan, Double::Nan);
			Assertv(!vec.isNan(), verbose);
			Assertv(vec.hasNan(), verbose);
			Assertv(!vec.hasNoNan(), verbose);
		}
		{
			const Vec3d vec(0.6, Double::Nan, 5.5);
			Assertv(!vec.isNan(), verbose);
			Assertv(vec.hasNan(), verbose);
			Assertv(!vec.hasNoNan(), verbose);
		}
	...
	{ // vector operations
		...
		{ // euler angle
			Vec3d::DefaultAxisRule = Rules::Axis::AXIS_RHS;
			Assertve(Vec3d::DefaultAxisRule == Rules::Axis::AXIS_RHS, verbose);
			{ // RHS
			...
			Vec3d::DefaultAxisRule = Rules::Axis::AXIS_LHS;
			Assertve(Vec3d::DefaultAxisRule == Rules::Axis::AXIS_LHS, verbose);
			{ // LHS
				Assertv(Vec3d(0, 0, 0).euler().equals(0.0, 0.0, 0.0), verbose);
