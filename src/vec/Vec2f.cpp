...
float Vec2f::fullAngle(const Vec2f& ref_vec, Rules::Axis axis_rule) const
{
	...
	{
		default:
		case Rules::Axis::AXIS_RHS: return std::fmod((F_2_PI + std::atan2(y, x) - std::atan2(ref_vec.y, ref_vec.x)), F_2_PI);
		case Rules::Axis::AXIS_LHS: return std::fmod((F_2_PI + std::atan2(x, y) - std::atan2(ref_vec.x, ref_vec.y)), F_2_PI);
	}
}
...
Vec2f Vec2f::rotated(float theta_z, Rules::Axis axis_rule) const
{
	...
	{
		default:
		case Rules::Axis::AXIS_RHS: return Vec2f((x * std::cos(theta_z) - y * std::sin(theta_z)), (x * std::sin(theta_z) + y * std::cos(theta_z)));
		case Rules::Axis::AXIS_LHS:
			theta_z = -theta_z;
...
Rules::Axis Vec2f::DefaultAxisRule = Rules::Axis::AXIS_RHS;
