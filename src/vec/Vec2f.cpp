...
float Vec2f::fullAngle(const Vec2f& ref_vec, Rules::Axis axis_rule) const
{
	...
	{
		default:
		case Rules::Axis::RHS: return std::fmod((F_2_PI + std::atan2(y, x) - std::atan2(ref_vec.y, ref_vec.x)), F_2_PI);
		case Rules::Axis::LHS: return std::fmod((F_2_PI + std::atan2(x, y) - std::atan2(ref_vec.x, ref_vec.y)), F_2_PI);
	}
}
...
Vec2f& Vec2f::rotate(float theta_z, Rules::Axis axis_rule)
{
	...
	switch (axis_rule)
	{
		...
		case Rules::Axis::RHS:
			{
				const float _x = (x * std::cos(theta_z) - y * std::sin(theta_z));
				const float _y = (x * std::sin(theta_z) + y * std::cos(theta_z));
				this->x = _x;
				this->y = _y;
			}
			return *this;
		case Rules::Axis::LHS:
			{
				const float _x = (x * std::cos(theta_z) + y * std::sin(theta_z));
				const float _y = (y * std::cos(theta_z) - x * std::sin(theta_z));
				this->x = _x;
				this->y = _y;
			}
			return *this;
...
Vec2f Vec2f::rotated(float theta_z, Rules::Axis axis_rule) const
{
	...
	{
		default:
		case Rules::Axis::RHS: return Vec2f((x * std::cos(theta_z) - y * std::sin(theta_z)), (x * std::sin(theta_z) + y * std::cos(theta_z)));
		case Rules::Axis::LHS:
			theta_z = -theta_z;
...
Rules::Axis Vec2f::DefaultAxisRule = Rules::Axis::RHS;
